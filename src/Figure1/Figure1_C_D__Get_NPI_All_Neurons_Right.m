...
for i=1:1:length(uniqueSuperclass)
    ...
    scatter(FAFBNPIs.Right_PostPI(idx),FAFBNPIs.Right_PrePI(idx),27,colors(i,:),"filled",'MarkerFaceAlpha',0.65);
    hold on;
...
xlim([-1.05 1.05])
ylim([-1.05 1.05])
% print(gcf,'-depsc2','-vector','figure1_Dots.eps')

%%
RightFF_by_type=FAFBNPIs_by_type((FAFBNPIs_by_type.Mean_Right_PostPI-FAFBNPIs_by_type.Mean_Right_PrePI)>=0.2,:);
RightFB_by_type=FAFBNPIs_by_type((FAFBNPIs_by_type.Mean_Right_PostPI-FAFBNPIs_by_type.Mean_Right_PrePI)<=-0.2,:);
RightBD_by_type=FAFBNPIs_by_type(((FAFBNPIs_by_type.Mean_Right_PostPI-FAFBNPIs_by_type.Mean_Right_PrePI)<0.2)&((FAFBNPIs_by_type.Mean_Right_PostPI-FAFBNPIs_by_type.Mean_Right_PrePI)>-0.2),:);


RightFF_NPIs=FAFBNPIs(ismember(FAFBNPIs.type,RightFF_by_type.type),:);
RightFB_NPIs=FAFBNPIs(ismember(FAFBNPIs.type,RightFB_by_type.type),:);
RightBD_NPIs=FAFBNPIs(ismember(FAFBNPIs.type,RightBD_by_type.type),:);
...
for i=1:1:length(uniqueSuperclass)
    ...
    if any(idx)
        ...
        % 히스토그램 계산
        % edges = -2:0.2:2; % 구간 정의
        % [N, edges] = histcounts(FAFBNPIs.Right_PostPI(idx)+FAFBNPIs.Right_PrePI(idx), edges, 'Normalization', 'probability');
...
xlim([-2.05 2.05])
ylim([0 1])
% print(gcf,'-depsc2','-vector','figure1_Plus.eps')
...
xlim([-2.05 2.05])
ylim([0 1])
% print(gcf,'-depsc2','-vector','figure1_Minus.eps')
...
for i=1:1:length(uniqueSuperclass)
    ...
    hold on;
    if any(idx)
        % h=histogram(FAFBNPIs.Right_PostPI(idx),-2:0.1:2,'Normalization','probability');
        % h.FaceColor=colors(i+3,:);
        % h.EdgeColor=colors(i+3,:);
        ...
        % 히스토그램 계산
        edges = -1:0.2:1; % 구간 정의
        [N, edges] = histcounts(FAFBNPIs.Right_PostPI(idx), edges, 'Normalization', 'probability');
...
% xlim([-1 1])
% ylim([0 1])
% print(gcf,'-depsc2','-vector','figure1_PostPI.eps')
...
for i=1:1:length(uniqueSuperclass)
    ...
    hold on;
    if any(idx)
        % h=histogram(FAFBNPIs.Right_PostPI(idx),-2:0.2:2,'Normalization','probability');
        % h.FaceColor=colors(i+3,:);
        % h.EdgeColor=colors(i+3,:);
        % h.FaceAlpha = 0.5; % 반투명 효과 (0: 완전 투명, 1: 불투명)
        ...
        % 히스토그램 계산
        edges = -1:0.2:1; % 구간 정의
        [N, edges] = histcounts(FAFBNPIs.Right_PrePI(idx), edges, 'Normalization', 'probability');
...
% xlim([-1 1])
ylim([0 1])
% print(gcf,'-depsc2','-vector','figure1_PrePI.eps')
...
for i=1:1:length(uniqueSuperclass)
    WantToSee=uniqueSuperclass{i}
    idx=strcmp(FAFBNPIs.superclass,WantToSee);
    scatter(FAFBNPIs.Right_PostPI(idx)+FAFBNPIs.Right_PrePI(idx),FAFBNPIs.Right_PostPI(idx)-FAFBNPIs.Right_PrePI(idx),27,colors(i,:),"filled",'MarkerFaceAlpha',0.65);
    hold on;
