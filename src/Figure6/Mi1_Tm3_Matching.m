...
%%
Tm3_Columns=table(FAFBConsolidated_type.root_id(strcmp(FAFBConsolidated_type.primary_type,'Tm3'),:),'VariableNames',{'root_id'});
load('Mi1_voxels.mat')

%%
zeroIdx=[];

for i=1:1:size(Tm3_Columns,1)
    current_Tm3=Tm3_Columns.root_id(i);
    upstreamConnections=FAFBConnections(FAFBConnections.post_root_id==current_Tm3,:);
    ...
    Tm3_Columns.upstreamMi1{i}=upstreamMi1;
    Tm3_Columns.hemisphere(i)=upstreamMi1.hemisphere(1);
    Tm3_Columns.theta{i}=[upstreamMi1.theta upstreamMi1.syn_count/sum(upstreamMi1.syn_count)];
    Tm3_Columns.phi{i}=[upstreamMi1.phi upstreamMi1.syn_count/sum(upstreamMi1.syn_count)];
end
Tm3_Columns(zeroIdx,:)=[];
...
for i=1:1:size(Tm3_Columns,1)
    swc_names=[swc_location num2str(Tm3_Columns.root_id(i)) '.swc'];
    [swc_position]=SWC_XYZ(swc_names);
    Tm3_Columns.voxel_position{i}=swc_position;
...
%% Tm3 voxel들 중 Lobula에 있는 것만..
zeroIdx=[];
for i=1:1:size(Tm3_Columns,1)
    currentVoxels=Tm3_Columns.voxel_position{i};
    currentVoxels_array=table2array(currentVoxels);

    VoxelsInLobula=currentVoxels(intriangulation(Lo_L_Vertices,Lo_L_Faces,currentVoxels_array)|intriangulation(Lo_R_Vertices,Lo_R_Faces,currentVoxels_array),:); % LO LR 바뀐듯
    Tm3_Columns.voxel_position_Lobula{i}=VoxelsInLobula;
    if isempty(VoxelsInLobula)
        zeroIdx=[zeroIdx i];
    end
end
Tm3_Columns(zeroIdx,:)=[];
...
zeroIdx=[];
for i=1:1:size(Tm3_Columns,1)
    current_root_id=Tm3_Columns.root_id(i);
    ...
    out_syn_loc=out_syn_loc((intriangulation(Lo_R_Vertices,Lo_R_Faces,out_syn_temp)|intriangulation(Lo_L_Vertices,Lo_L_Faces,out_syn_temp)),:); % LO LR 바뀐듯
    if isempty(out_syn_loc)
        zeroIdx=[zeroIdx i];
        continue;
    end

    Tm3_Columns.out_syn_loc{i}=out_syn_loc;
end
Tm3_Columns(zeroIdx,:)=[];
...
out_syn_loc_L=[];
for i=1:1:size(Tm3_Columns,1)
    if isempty(Tm3_Columns.out_syn_loc{i})
        continue;
    elseif strcmp(Tm3_Columns.hemisphere{i},'R')
        out_syn_loc_R=[out_syn_loc_R;table2array(Tm3_Columns.out_syn_loc{i})];
    elseif strcmp(Tm3_Columns.hemisphere{i},'L')
        out_syn_loc_L=[out_syn_loc_L;table2array(Tm3_Columns.out_syn_loc{i})];
    end
end
...
for i=1:1:size(Tm3_Columns,1)
    ...
    current_out_syn_loc_Denoise=table2array(Tm3_Columns.out_syn_loc{i});
    ...
    Tm3_Columns.out_syn_loc_denoise{i}=table(current_out_syn_loc_Denoise(idx,1),current_out_syn_loc_Denoise(idx,2),current_out_syn_loc_Denoise(idx,3),'VariableNames',{'x','y','z'});
end
Tm3_Columns(zeroIdx,:)=[];
%% ===================== 설정 =====================
...
%%% ===================== 메인 루프 =====================
for i = 1:height(Tm3_Columns)
    %%% 0) 출력 시냅스 없으면 스킵 (나중에 행 삭제)

    swcPath=[swc_location num2str(Tm3_Columns.root_id(i)) '.swc'];

    % 1) 입력 데이터 꺼내기
    A = table2array(Tm3_Columns.voxel_position_Lobula{i,1});  % [M x 3] 복셀 좌표
    B = table2array(Tm3_Columns.out_syn_loc_denoise{i,1});             % [K x 3] 시냅스 좌표

    % 방어: 비어있으면 빈 저장 후 continue
    if isempty(A) || isempty(B) || ~isfile(swcPath)
        Tm3_Columns.ancestors_outsyn{i} = table([],[],[], 'VariableNames',{'x','y','z'});
    ...
    Tm3_Columns.ancestors_outsyn{i} = array2table(V, 'VariableNames', {'x','y','z'});
