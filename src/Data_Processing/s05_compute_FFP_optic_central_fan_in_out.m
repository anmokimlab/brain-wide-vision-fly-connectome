...
[UniqueRightFFP_type,~,ic]=unique(RightFFP_InOut.type);
UniqueRightFFP=table(UniqueRightFFP_type,'VariableNames',{'type'});


isEmptyIn = cellfun(@(c) isempty(c) || all(cellfun(@isempty, c(:))), RightFFP_InOut.In);
isEmptyOut = cellfun(@(c) isempty(c) || all(cellfun(@isempty, c(:))), RightFFP_InOut.Out);

for i=1:1:size(UniqueRightFFP,1)
    idx=ic==i;
    EIn_N=sum(idx&~isEmptyIn);
    EOut_N=sum(idx&~isEmptyOut);

    UniqueRightFFP.NofType(i)=sum(idx);
    if ~EIn_N==0

        tempInStats=RightFFP_InOut.In(idx);
        ...
        UniqueRightFFP.In{i}=tempInStats;
    end
    if ~EOut_N==0

        tempOutStats=RightFFP_InOut.Out(idx);
        ...
        UniqueRightFFP.Out{i}=tempOutStats;
    end
end
