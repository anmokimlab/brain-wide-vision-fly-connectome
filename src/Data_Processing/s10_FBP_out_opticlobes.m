...
RightFBP_Neuropils=seeConnection_by_region(RightFBP_type,FAFBConnections,FAFBConsolidated_type);
...
for i = 1:size(RightFBP_OutNeuropils,1)
    temp = RightFBP_Neuropils.OutNeuropils{i,1};
    idx_aMeL = strcmp(temp(:,1),'AME_L');
    idx_aMeR = strcmp(temp(:,1),'AME_R');
    idx_MeL = strcmp(temp(:,1),'ME_L');
    idx_MeR = strcmp(temp(:,1),'ME_R');
    idx_LoL = strcmp(temp(:,1),'LO_L');
    idx_LoR = strcmp(temp(:,1),'LO_R');
    idx_LoPL = strcmp(temp(:,1),'LOP_L');
    idx_LoPR = strcmp(temp(:,1),'LOP_R');
