...
% FBP (feedback projection) neuron meshes are downloaded by
% Data_Processing/s03_FFP_FBP_mesh.ipynb into Processed_Data/fig4a_neuron_mesh_FBP.
neuronDir     = fullfile(baseDir, 'Processed_Data', 'fig4a_neuron_mesh_FBP');
neuronAlpha   = 1.0;
neuronEdge    = 'none';
...
if ~isempty(vFilesNP)
    ...
    for i = 1:numel(vFilesNP)
        ...
        fPath = fullfile(vFilesNP(i).folder, strrep(vFilesNP(i).name, "_vertices.csv", "_faces.csv"));
        if ~isfile(fPath), continue; end

        V_np = readmatrix(vPath); F_np = readmatrix(fPath);
        if isempty(V_np) || isempty(F_np), continue; end

        F_np = round(F_np);
        if min(F_np(:)) == 0, F_np = F_np + 1; end

        trisurf(F_np, V_np(:,1), V_np(:,2), V_np(:,3), ...
            'EdgeColor','none', 'FaceColor', neuropilColor, ...
            'FaceAlpha', neuropilAlpha, 'FaceLighting', 'none');
    end
end
...
if isempty(vFilesNeu)
    ...
    for i = 1:numFiles
        ...
        if ~isempty(idStrList)
            ...
            if any(rowIdx)
                ...
                if iscell(t), neuronTypes{i} = char(t{1});
                elseif iscategorical(t), neuronTypes{i} = char(t);
                else, neuronTypes{i} = char(string(t));
                end
            else
    ...
    cmap = lines(max(10, numUnique));
    typeColorMap = containers.Map(uniqueTypes, num2cell(cmap(1:numUnique, :), 2));

    % --- Step 3: actually draw ---
    for i = 1:numFiles
        vPath = fullfile(vFilesNeu(i).folder, vFilesNeu(i).name);
        fPath = fullfile(vFilesNeu(i).folder, strrep(vFilesNeu(i).name, "_vertices.csv", "_faces.csv"));
        if ~isfile(fPath), continue; end

        V = readmatrix(vPath); F = readmatrix(fPath);
        if isempty(V) || isempty(F), continue; end

        V = double(V(:,1:3)); F = round(double(F(:,1:3)));
        if min(F(:)) == 0, F = F + 1; end

        % Index validity check
        maxIdx = size(V,1);
        valid = all(F >= 1 & F <= maxIdx, 2);
        if ~all(valid), F = F(valid,:); end

        % Color decision
        fc = singleColor;
        if ~useSingleColor, fc = typeColorMap(neuronTypes{i}); end

        trisurf(F, V(:,1), V(:,2), V(:,3), ...
            'FaceColor', fc, 'EdgeColor', neuronEdge, ...
            'FaceAlpha', neuronAlpha, 'DisplayName', neuronTypes{i});
    end
end
...
delete(findall(gcf,'Type','light')); camlight('headlight'); lighting gouraud; material shiny;
