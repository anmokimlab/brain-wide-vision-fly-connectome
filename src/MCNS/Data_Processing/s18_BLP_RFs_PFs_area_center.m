%% s18 — RF/PF area & center metrics from the BLP RF/PF heatmaps  [MCNS]
% MCNS analogue of the FAFB Data_Processing/s18_BLP_RFs_PFs_area_center.m.
% Reads the per-type RF/PF heatmaps (all_BLP_by_type) from
% Processed_Data/BLP_RFs_PFs.mat (Figures/fig_6A_BLP_RFs_PFs.m) and, per neuron,
...
% Processed_Data/BLP_RFs_PFs_area_center.mat.

clear; clc;

baseDir = fileparts(fileparts(mfilename('fullpath')));  % resolved from this script's location (portable; no absolute paths)

matfile_in  = fullfile(baseDir, 'Processed_Data', 'BLP_RFs_PFs.mat');             % from fig_6A
...
% ---------- Grid / contour params (must match fig_6A) ----------
