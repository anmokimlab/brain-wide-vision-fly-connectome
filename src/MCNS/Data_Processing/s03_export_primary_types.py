#!/usr/bin/env python3
"""
Export a male-cns:v0.9 primary (consolidated) type table as CSV.

Output: MCNS/MCNS_Data/male-cns-v0.9-primary-types.csv
Output columns:
...
DEFAULT_OUTPUT = DATA_DIR / "male-cns-v0.9-primary-types.csv"
DEFAULT_TOKEN_FILE = BASE_DIR / ".neuprint_token"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export male-cns-v0.9-primary-types.csv from male-cns:v0.9."
    )
    parser.add_argument(
        "--server",
...
def fetch_consolidated_types_table(client: Client):
    q = """
        MATCH (n:Neuron)
        RETURN n.bodyId AS root_id,
               n.type AS primary_type,
...
def export_consolidated_types_table(client: Client, output_path: str) -> Path:
    output_file = Path(output_path)
    output_file.parent.mkdir(parents=True, exist_ok=True)

    df = fetch_consolidated_types_table(client)
...
def main() -> int:
    ...
    output_file = export_consolidated_types_table(client, args.output)
