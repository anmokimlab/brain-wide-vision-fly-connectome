%% Load data
% MCNS analogue of the FAFB Figures/fig_4D_FBP_dysynaptic_optclobe.m.
% Estimates each FBP type's disynaptic optic-lobe input along the path
...
opticlobes={'AME(R)','ME(R)','LO(R)','LOP(R)','AME(L)','ME(L)','LO(L)','LOP(L)'};
...
for i=1:1:size(FBP_Lop,1)
    ...
    [upstreamNeurons]=seeConnection_root_id_NoOptic(Wantrootids,MCNSConnections,MCNSConsolidatedTypes);
    ...
    for j=1:1:size(upstreamNeurons,1)
        ...
        idx=ismember(MCNSConnections.post_root_id,currentUpstream_root_id);

        InConnections_upstream=MCNSConnections(idx,:);
        Total_InConnections_upstream=sum(InConnections_upstream.syn_count);

        opticlobesSynapses(j,1)=(sum(InConnections_upstream.syn_count(strcmp(InConnections_upstream.neuropil,'AME(R)')))+sum(InConnections_upstream.syn_count(strcmp(InConnections_upstream.neuropil,'ME(R)'))))/Total_InConnections_upstream*upstreamNeurons(j,2);
        opticlobesSynapses(j,2)=(sum(InConnections_upstream.syn_count(strcmp(InConnections_upstream.neuropil,'AME(L)')))+sum(InConnections_upstream.syn_count(strcmp(InConnections_upstream.neuropil,'ME(L)'))))/Total_InConnections_upstream*upstreamNeurons(j,2);

        opticlobesSynapses(j,3)=sum(InConnections_upstream.syn_count(strcmp(InConnections_upstream.neuropil,'LO(R)')))/Total_InConnections_upstream*upstreamNeurons(j,2);
        opticlobesSynapses(j,4)=sum(InConnections_upstream.syn_count(strcmp(InConnections_upstream.neuropil,'LO(L)')))/Total_InConnections_upstream*upstreamNeurons(j,2);

        opticlobesSynapses(j,5)=sum(InConnections_upstream.syn_count(strcmp(InConnections_upstream.neuropil,'LOP(R)')))/Total_InConnections_upstream*upstreamNeurons(j,2);
        opticlobesSynapses(j,6)=sum(InConnections_upstream.syn_count(strcmp(InConnections_upstream.neuropil,'LOP(L)')))/Total_InConnections_upstream*upstreamNeurons(j,2);
...
%% Figure 1: mean +/- std disynaptic optic-lobe input per neuropil
...
%% Figure 2: Me-targeting FBP types, Me input (contra L vs ipsi R)
...
%% Figure 3: Lo-targeting FBP types, Lo input (contra L vs ipsi R)
...
%% Figure 4: Lop-targeting FBP types, LoP input (contra L vs ipsi R)
