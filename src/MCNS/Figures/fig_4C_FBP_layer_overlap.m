%% fig_4C_FBP_layer_overlap (MCNS)
% MCNS analogue of the FAFB Figures/fig_4G_FBP_layer_overlap.m.
% For each FBP neuron, Gaussian-smooths the output depth profile and the upstream
% input depth profiles (contralateral L / ipsilateral R, each scaled by its share
% of the total upstream synapses), then computes the overlap coefficient between
% them as the per-depth sum(min(...)).
...
%% Figure 1: per-neuron overlap, ME
...
%% Figure 2: per-neuron overlap, LO
...
%% Figure 3: per-neuron overlap, LOP
...
%% Figure 4: overlap distribution across all neurons, per region/side
